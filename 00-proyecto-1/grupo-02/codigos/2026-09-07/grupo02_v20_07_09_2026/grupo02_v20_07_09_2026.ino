...
const int ldrPin = A2; // pin analógico donde se conecta el LDR
int ldrLectura = -1;
const int umbralLuzLDR = 200; // rango 0–1023, esto se puede cambiar dependiendo del ambiente en el que estemos
                              // hoy, dentro de LID a las 11:46 AM funciona con el umbral a 200
...
const unsigned long intervaloAvanceLuz = 200; // ms de luz sostenida antes de avanzar una línea
...
// animaciones: líneas gatillantes y tolerancia HOLA HOLA HOLA ESTO SE VA??????????????????????????????????????????????????????????????????????????????'

// líneas (contando los \n del poemaLuz) que gatillan cada animación
// esto pasa cuando el scroll controlado por LDR se queda quieto en ellas
// por al menos "tiempoQuietoAnimacion" ms (se puede ajustar)
...
const int windowHeight = visibleLines * lineHeight; // alto real de la ventana visible (16 px, equivalente a 2 líneas)
...
// bitmaps!!!!!!!!!!!!!!!!!!! HOLA ESTO HAY QUE BORRARLO???????????????????????????????????????????????????????

// 'abeja', 128x64px — imagen para la animación 1 ("los Romeros florecen")
const unsigned char epd_bitmap_abeja [] PROGMEM = {
...
void mostrarAnimacion1() {
  // imagen fija de la abeja, ocupando toda la pantalla (128x64)
  display.drawBitmap(0, 0, epd_bitmap_abeja, SCREEN_WIDTH, SCREEN_HEIGHT, SH110X_WHITE);
}
