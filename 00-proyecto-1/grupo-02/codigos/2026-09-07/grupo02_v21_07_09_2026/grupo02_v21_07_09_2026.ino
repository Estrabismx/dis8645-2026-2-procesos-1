...
#include <Adafruit_SH110X.h>   // <-- CAMBIO: antes era Adafruit_SSD1306.h
...
// CAMBIO: el objeto ya no es Adafruit_SSD1306, ahora es Adafruit_SH1106G
...
int ldrLectura = -1;
const int umbralLuzLDR = 250; // rango 0–1023, esto se puede cambiar dependiendo del ambiente en el que estemos
...
// línea (contando los \n del poemaLuz) que dispara la animación del colibrí,
...
// también cuenta.
// IMPORTANTE: como el scroll de poemaLuz avanza de a 2 líneas, "lineaActualLuz"
// solo toma valores PARES (0, 2, 4, 6...) — por eso tolLinea NO puede ser 0.
...
//ella pueda procesarlo y resistir, y nosotres actuar.
  ...
  // CAMBIO: display.begin() ahora recibe (dirección I2C, reset).
  ...
  // CAMBIO: SSD1306_WHITE -> SH110X_WHITE (la librería SH110X usa su propio nombre de color)
  ...
  // CAMBIO: la librería SH110X no tiene "ssd1306_command()", pero sí tiene
  // su propio "oled_command()" (heredado de la clase base Adafruit_GrayOLED),
  // que funciona igual: recibe un comando de bajo nivel, en este caso
  // SH110X_DISPLAYOFF en vez de SSD1306_DISPLAYOFF.
  // Se queda así hasta la primera presión del botón (ver loop()).
  display.oled_command(SH110X_DISPLAYOFF); // apaga físicamente el panel OLED
}
  ...
  void loop() {
      ...
      if (lecturaBoton != estadoBotonEstable) {
        ...
        if (estadoBotonEstable == HIGH) { // high si el botón fue presionado
          ...
          if (pantallaEncendida) {
            display.oled_command(SH110X_DISPLAYON); // CAMBIO: prendemos la pantalla

            } else {
            display.oled_command(SH110X_DISPLAYOFF); // CAMBIO: apagamos la pantalla
    ...
    if (mostrarPoemaLuz) {
      ...
      // avanza de a "windowHeight" (2 líneas) para no repetir ninguna línea
      if (hayLuzLDR) {
      ...
      if (quietaSuficiente && cercaDe(lineaActualLuz, lineaTrasColibries)) {
        mostrarAnimacion2();
  ...
  void mostrarTextoConScroll(const char* texto, int maxScrollTexto) {
    ...
    // (antes avanzaba de a 1 línea, por lo que la línea del medio se repetía)
  ...
  void dibujarTextoEnScroll(const char* texto, int scrollY) {
    ...
    // CAMBIO: SSD1306_BLACK -> SH110X_BLACK
...
void mostrarAnimacion2() {
  ...
  const float velocidad = 0.8; // velocidad fija (entre 0.2 y 1.0);
  ...
  // --- aleteo: alterna arriba/abajo cada cierto intervalo ---
  ...
  // --- notas musicales que salen del pico ---
