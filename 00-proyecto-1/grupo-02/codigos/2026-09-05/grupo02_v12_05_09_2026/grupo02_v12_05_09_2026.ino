...
// el LDR nos muestra las animaciones cuando le llega luz directa,
// eso es solo si se estamos mostrando poemaLuz, 
// si estamos en poema1, no sucede nada
...
// líneas (contando los \n del poemaLuz) justo después de cada verso
// cuando el potenciómetro 1 deja esa línea como la primera visible
// de la ventana, se considera como que ya pasamos ese verso
// por lo que no se mostrará la animación
...
//ella pueda procesarlo y resistir, y nosotres actuar.
  ...
  void loop() {
    ...
    if (lecturaBoton != estadoBotonAnterior) {
      ultimoCambioBoton = millis(); // la lectura cambió, por lo que reiniciamos el conteo de debounce
    }
    ...
    // decidimos qué texto mostrar según el mismo potenciómetro que controla el LED
    // LED con poca intensidad (pote2Lectura bajo), se muestra el texto alternativo (poemaLuz)
    // LED intensa (pote2Lectura alto), se muestra solo el texto inicial (poema1)
    bool mostrarPoemaLuz = pote2Lectura < umbralPote2;

    // leer y filtrar el primer potenciómetro (se usa para el scroll, en ambos modos de lectura)
    poteLectura = analogRead(potePatita);
    poteFiltrado = filtrarConDivision(poteLectura, 4); // 0–255 approx

    // lectura del LDR: solo importa cuando se está mostrando poemaLuz
    ...
    if (mostrarPoemaLuz) {
      // calculamos a qué línea de poemaLuz corresponde el scroll actual,
      // para saber si toca disparar alguna animación
      int scrollY = map(poteFiltrado, 0, 255, 0, maxScrollLuz);
      scrollY = (scrollY / lineHeight) * lineHeight;
      int lineaActual = scrollY / lineHeight;

      if (hayLuzLDR && lineaActual == lineaTrasRomeros) {
        mostrarAnimacion1();
        Serial.println("LDR con luz + poemaLuz -> animación 1 (Romeros)");
      } else if (hayLuzLDR && lineaActual == lineaTrasColibries) {
        mostrarAnimacion2();
        Serial.println("LDR con luz + poemaLuz -> animación 2 (Colibries)");
      } else if (hayLuzLDR && lineaActual == lineaTrasGrandecito) {
        mostrarAnimacion3();
        Serial.println("LDR con luz + poemaLuz -> animación 3 (grandecito)");
      } else if (hayLuzLDR && lineaActual == lineaTrasRevelandose) {
        mostrarAnimacion4();
        Serial.println("LDR con luz + poemaLuz -> animación 4 (revelandose de dia)");
      } else {
        // LED con baja intensidad mostramos poemaLuz
        // con el mismo scroll
        // centrado (offset)
        // y tamaño que el poema1
        mostrarTextoConScroll(poemaLuz, maxScrollLuz);

        Serial.println("mostrando poemaLuz (LED baja intensidad)");
      }
    } else {

      // con LED intensa mostramos el poema principal
      // (el LDR nunca hace nada acá, aunque le llegue la luz directa)
  ...
  // esto aplica scroll (según el primer potenciómetro)
  // centra el bloque de "visibleLines" en la pantalla
  // y tapa todo lo que quede fuera de esa ventana central
  // por lo que se usa tanto para poema1 como para poemaLuz.
  void mostrarTextoConScroll(const char* texto, int maxScrollTexto) {
    ...
    // se desplaza el texto lolololol
    display.setCursor(0, offsetY - scrollY); // offsetY centra el bloque de "visibleLines",
...
// Animación 2: figura que "vuela" de un lado a otro (para "los Colibries cantan")
...
// Animación 3: anillos concéntricos expandiéndose (para "que es grandecito")
...
// Animación 4: "sol" que sube desde abajo (para "revelandose de dia")
