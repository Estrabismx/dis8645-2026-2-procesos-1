...
const unsigned long intervaloAvanceLuz = 400; // ms de luz sostenida antes de avanzar una línea
...
const unsigned long tiempoQuietoAnimacion = 4000; // ms que debe estar quieta la línea para animar
...
//ella pueda procesarlo y resistir, y nosotres actuar.
  ...
  void loop() {
      ...
      if (lecturaBoton != estadoBotonEstable) {
        ...
        if (estadoBotonEstable == HIGH) { // flanco de subida = botón recién presionado
          ...
          if (pantallaEncendida) {
            ...
            poemaLuzActivado = false; // reiniciamos: la próxima vez hay que
                                       // volver a bajar el LED una vez para activar poemaLuz
    ...
    // una vez que el LED baja lo suficiente, poemaLuz queda "enganchado":
    // ya no vuelve a poema1 aunque el LED suba de intensidad después.
    // Desde ese momento, pote2 solo controla el brillo del LED (arriba),
    // sin afectar más lo que se ve en pantalla.
    if (!poemaLuzActivado && pote2Lectura < umbralPote2) {
      poemaLuzActivado = true;
    }
    bool mostrarPoemaLuz = poemaLuzActivado;

    // leer y filtrar el primer potenciómetro (se usa para el scroll de poema1)
    ...
    if (mostrarPoemaLuz) {
      ...
      if (hayLuzLDR) {
        ...
        // sin luz: no avanza. Reiniciamos el temporizador para que no
        // "salte" varias líneas de golpe apenas vuelva la luz.
        ultimoAvanceLuz = millis();
      }

      int lineaActualLuz = scrollYLuz / lineHeight;

      // si la línea actual cambió, reiniciamos el conteo de "tiempo quieto"
      ...
      if (quietaSuficiente && lineaActualLuz == lineaTrasRomeros) {
        mostrarAnimacion1();
        Serial.println("4+ seg quieto tras Romeros -> animación 1");
      } else if (quietaSuficiente && lineaActualLuz == lineaTrasColibries) {
        mostrarAnimacion2();
        Serial.println("4+ seg quieto tras Colibries -> animación 2");
      } else if (quietaSuficiente && lineaActualLuz == lineaTrasGrandecito) {
        mostrarAnimacion3();
        Serial.println("4+ seg quieto tras grandecito -> animación 3");
      } else if (quietaSuficiente && lineaActualLuz == lineaTrasRevelandose) {
        mostrarAnimacion4();
        Serial.println("4+ seg quieto tras revelandose de dia -> animación 4");
      } else {
  ...
  // centra el bloque de "visibleLines" en la pantalla, y tapa todo lo que
  ...
  // dibuja "texto" con un scrollY YA CALCULADO (no lo deriva de ningún
  // potenciómetro): centra el bloque de "visibleLines" y tapa lo que quede
  // fuera de esa ventana. La usan tanto mostrarTextoConScroll() (para poema1,
  // con scrollY del potenciómetro 1) como el modo poemaLuz (con scrollYLuz,
  // que ahora lo controla el LDR en vez de un potenciómetro).
